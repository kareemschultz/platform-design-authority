...
5. **Theme and preset changes are never casual refinement.** Installing a Studio theme (`@ss-themes`, or `/rui`'s theme-install capability) or changing base color, fonts, radius, or density is governed by `docs/blueprint/09-UX/SHADCN_CONFIGURATION_DECISION_MATRIX.md` — a §8 design-system decision requiring review, not a per-surface tweak an agent makes inline.
