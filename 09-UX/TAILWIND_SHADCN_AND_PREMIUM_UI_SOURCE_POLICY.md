...
## Canonical shadcn Bootstrap

`SHADCN_CONFIGURATION_DECISION_MATRIX.md` governs the initial configuration for new authenticated web applications and `packages/ui-web`:

- Base UI component base
- Rhea style, with Nova as the tested fallback
- Neutral generator base color
- Blue default action/theme and chart seed
- Geist Sans headings, Inter Variable body, and constrained Geist Mono use
- Lucide icons
- Default radius, subtle menu accent, and default menu color
- CSS variables, TSX, Next.js RSC configuration, RTL, and pointer cursor enabled

These are bootstrap inputs, not permanent raw design tokens. Generated values are normalized into `registry/design-tokens.json`, owned component variants, and platform accessibility contracts. Do not mix multiple shadcn style families inside the same owned component package.
...
- Store the human-readable preset inputs and the decoded code generated by the pinned CLI; do not treat a screenshot or opaque code as sufficient provenance.
