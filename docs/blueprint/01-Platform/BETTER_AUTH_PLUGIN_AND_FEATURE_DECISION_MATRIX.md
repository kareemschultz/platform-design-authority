...
| MCP | Defer; expected successor is OAuth 2.1 Provider | Governed Developer Platform authorization-server and MCP design | Better Auth documents the MCP plugin as headed toward deprecation in favor of the OAuth Provider plugin. Authentication is only one layer; discovery and execution still require canonical commands, permissions, entitlements, approvals, tenant scope, audit, and AI safety policy. |
