...
- Where a virtualized collection needs list/grid keyboard and selection semantics beyond what TanStack Table/Virtual provide, `ADR-0022-BASE-UI-BACKED-SHADCN-PRIMITIVES.md`'s React Aria hook exception supplies that behavior layer, composed with — not replacing — TanStack Virtual's DOM windowing: the hooks own selection/keyboard/ARIA, TanStack Virtual owns which rows are actually rendered. For a genuinely single-column, non-tabular collection, `@react-aria/gridlist`+`@react-stately/list` (the ARIA grid pattern) is correct when rows contain interactive children (buttons, checkboxes, menus); `@react-aria/listbox`+`@react-stately/list` (the ARIA listbox pattern) is for single-column, static-content-only rows.
- **For an actual multi-column data grid/table** — headers, cells, column navigation, sorting — `@react-aria/table`+`@react-stately/table` is the correct family; React Aria's own docs describe `GridList` as a single-column collection without header, column, or sort infrastructure, so it drops those semantics for exactly the tabular case this document governs (a Codex review caught an earlier draft of this line routing tabular grids to `gridlist` instead). **State ownership is not yet decided**, and this document does not claim it is: React Aria's `useTableState` builds its own table collection and manages selection and sort internally, which directly overlaps TanStack Table's existing ownership of sorting, filtering, pagination, visibility, and selection state named above — using both as-is would give one grid two competing state authorities (a Codex review caught this contradiction). TanStack Table remains the platform's single state authority for sort/filter/pagination/visibility/selection; if the required prototype (see `ADR-0022-BASE-UI-BACKED-SHADCN-PRIMITIVES.md`'s Validation section) adopts the React Aria table hooks, it must resolve and record the exact adapter — driving `useTable`'s keyboard/ARIA layer from TanStack Table's external state rather than `useTableState`'s own — before this becomes implementation guidance; a real Base UI `Table` composition without React Aria's table hooks remains the default absent that resolution.
