...
2. Virtualized-collection keyboard and selection behavior — composed with the platform's existing TanStack Virtual windowing commitment (`ENTERPRISE_TABLE_AND_DATA_GRID_STANDARD.md`), not in place of it; the hooks supply selection/keyboard/ARIA semantics, TanStack Virtual supplies DOM windowing. Use `@react-aria/listbox` for static-content collections and `@react-aria/gridlist` for collections whose rows contain interactive children (buttons, checkboxes, menus) — these are different ARIA role/keyboard contracts, not interchangeable.
