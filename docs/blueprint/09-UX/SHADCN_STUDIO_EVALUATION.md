...
Website evidence was verified in rendered Chrome sessions with the browser's Playwright-backed DOM inspection and viewport screenshots, not inferred only from static page extraction or URL parameters. The Autocomplete page exposes an explicit Base UI/Radix UI component-library selector. A 2026-07-18 recheck at `?base=radix` showed `Radix UI Component Library` selected together with `/preview/components/radix/autocomplete?index=0`; an earlier rendered read exposed `Base UI Component Library` at the same URL. These observations are consistent with different selector, client-navigation, or persisted presentation state and do not demonstrate a renderer defect. The page also renders support and explanatory prose for both libraries, so evidence capture must record the selected control, URL, and preview link together. Those presentation signals still do not establish the copied item's source dependency graph. Screenshots are ephemeral review evidence and are not committed because they reproduce third-party presentation content; the durable repository evidence is the bounded observation and source URL below.
...
| Public component catalog | The official page advertises 58 component families, 1,000+ variants, and a new Autocomplete family with 10 components. The page exposes an explicit Base UI/Radix UI selector; the verified Radix presentation combined `Radix UI Component Library`, `?base=radix`, and `/preview/components/radix/autocomplete?index=0`, while an earlier read exposed the other selector label and the page retained support prose for both libraries | These are vendor catalog and presentation claims, not an acceptance, accessibility, source, or dependency result. Capture each selector state atomically with its URL and preview link; neither the selector nor explanatory copy alters Meridian's governed primitive choice or proves the acquired item's implementation |
...
|---|---|---|
| Autocomplete 10 | Researching | Compare visual and interaction ideas only against the official shadcn Command/Combobox and the platform-owned searchable-select direction. New owned work remains Base UI-backed under ADR-0022; the Studio Radix view is comparison/fallback evidence only and does not authorize mixing primitive families. Because Studio's explicit selector can present Base UI or Radix UI states and its explanatory copy covers both, capture the active selector, URL, and preview link together and require item-level source and dependency evidence before assigning a primitive family. Any prototype must preserve tenant and permission filtering, cancellable/debounced remote search, no-results, stale, degraded, offline, large-data, keyboard, focus, announcement, and custom-option semantics. |
