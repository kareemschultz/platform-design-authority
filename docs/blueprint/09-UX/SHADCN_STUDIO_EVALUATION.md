...
| Public component catalog | The official page advertises 58 component families, 1,000+ variants, and a new Autocomplete family with 10 components; its documentation exposes Base UI and Radix UI modes, including the `?base=radix` view | These are vendor catalog claims, not an acceptance or accessibility result; the mode selector does not alter Meridian's governed primitive choice |
