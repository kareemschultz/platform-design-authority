...
| `packages/persistence/platform-import-export-postgres` | `platform.import-export` | `@meridian/platform-import-export` | `platform_import_job`, `platform_import_row`, `platform_import_finding`, `platform_import_wave`, `platform_import_command_receipt`, `platform_export_job` | `packages/persistence/platform-import-export-postgres/src/migrations` |
...
| `packages/persistence/pos-postgres` | `pos` | `@meridian/domain-pos` | `pos_register_session`, `pos_cash_movement`, `pos_command_receipt`, `pos_sale`, `pos_sale_line`, `pos_price_override`, `pos_receipt`, `pos_return`, `pos_return_line`, `pos_refund`, `pos_deposit`, `pos_deposit_source_shift`, `pos_deposit_custody_transfer` | `packages/persistence/pos-postgres/src/migrations` |
...
- 1.12.0 (2026-07-18): WS3 PR4 (controlled prototype, `claude/ws3-integration`) adds `pos_deposit`, `pos_deposit_source_shift`, and `pos_deposit_custody_transfer` to `packages/persistence/pos-postgres`'s owned tables (Deposit maker/checker — commerce.deposit.create/.confirm, frozen control plan §6.6) and `platform_export_job` to `packages/persistence/platform-import-export-postgres`'s owned tables (the accountant-handoff export artifact — `platform.export.create`/`.read`). No new package or owner is registered: both packages already existed with an established owner (`pos` / `platform.import-export`) since PR0/PR1; this stage only adds tables under those existing owners. `packages/platform/import-export` declares its own local `AccountantHandoffSourceData` structural port (never importing `@meridian/domain-pos`) so the export builder stays in the `platform` package family without crossing the `family_grants_are_contract_only` boundary §10.4 already records for `domains` -> `engines`; `apps/server/composition` (a registered composition root) is the only place a real `posService.queryFinanceHandoffSourceData(...)` result is passed to it.
- 1.11.0 (2026-07-18): WS3 PR3 (controlled prototype, `claude/ws3-integration`) adds `pos_return`, `pos_return_line`, and `pos_refund` to `packages/persistence/pos-postgres`'s owned tables (Return, Refund — commerce.return.create/.approve, commerce.refund.create/.approve, commerce.receipt.void/.reissue). `packages/persistence/pos-postgres`'s existing `pos_sale_line` table gains an `inventory_movement_id` column (no new table) so a Return's compensating movement can satisfy Inventory's `reversalOfMovementId` traceability requirement. Separately, `packages/persistence/inventory-postgres`'s existing `inventory_stock_movement_tenant_reversal_uidx` partial unique index is rescoped to exclude `source_type = 'Sale'` rows (no new table, no new migration-owned table row): the index previously capped ANY original movement at exactly one `Reversal` ever, which is correct for `reverseAdjustment`'s single-full-reversal semantics but incorrectly blocked WS3's legitimate case of multiple, separate partial Returns compensating the SAME original Sale movement over time. The cumulative-quantity cap for Sale-sourced reversals is enforced by `@meridian/domain-pos`'s own Sale-row-locked `sumReturnedQuantity` check, not by this index.
