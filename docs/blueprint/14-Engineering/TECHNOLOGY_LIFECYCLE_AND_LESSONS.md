...
| TECH-LESSON-029 | 2026-07-13 | Active | `shadcn-studio-mcp@1.0.7` (confirmed by reading its `build/utils/config.js` source) only recognizes the environment variables `API_KEY` and `EMAIL`; a client configuration naming the license variable anything else (observed: a separate `.codex/config.toml` entry using `LICENSE_KEY`) leaves `apiKey` unset and the package silently runs in freemium mode | Configure Studio MCP credentials only as `API_KEY`/`EMAIL`; a successful tool call is not evidence of authenticated access — verify entitlement with a with/without-credential comparison, not by whether the call succeeds | Source-read of `build/utils/config.js`; controlled comparison returned 1 variant unauthenticated vs. 18 authenticated (3 `isPro: true`) for the same call | Frontend Platform | Every Studio MCP release or client-config naming change |
...
| TECH-LESSON-031 | 2026-07-13 | Active | The Claude Code desktop application embeds a full JSON snapshot of the resolved MCP server configuration (including whatever was in `args`/`env` at the time) into its own host process's command line at launch, and does not appear to refresh this snapshot for an already-running process when `~/.claude.json` is edited or a new chat/session is started within the same running app | A newly-added MCP server (or a credential/version fix to an existing one) does not take effect until the desktop application itself is fully quit and relaunched, not merely a new conversation; this also means the host process's own command line can expose a stale credential in its old raw-argument form until that relaunch happens | Two live `claude.exe` processes (one of them this session's own host process) were found with the pre-migration Studio config, including the raw-argument credential, embedded in their command line via `Get-CimInstance Win32_Process`, well after the on-disk config had been corrected | Developer Platform | Every MCP config change; recheck after the user confirms a full app relaunch |
