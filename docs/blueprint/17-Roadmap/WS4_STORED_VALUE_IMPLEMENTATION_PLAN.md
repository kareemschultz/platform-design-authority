...
| `packages/contracts/platform-api` (extend) | Generated stored-value oRPC contract surface | Generated; not hand-edited |
| `packages/contracts/events` (extend) | Stored-value event payload types, zod v4 boundary schemas | Generated from `registry/events.json` + `schemas/events/`; not hand-edited |
...
- **Separation of duties.** PDA-DOM-025 §Security and Fraud Controls requires "separation of administration, adjustment, and reconciliation." This plan maps that requirement onto the registered/proposed permission set rather than inventing a dedicated administration permission: **administration** is the program/instrument lifecycle group (`commerce.stored-value.issue`, `.load`, `.suspend` — creating and configuring what exists to be adjusted or reconciled); **adjustment** is the request/approve/deny pair (`commerce.stored-value.adjust`, `.adjustment-approve` proposed); **reconciliation** is `commerce.stored-value.reconcile`. Tests prove a single fixture role cannot hold administration and adjustment-approval together, cannot hold adjustment-request and adjustment-approval together, and cannot hold adjustment and reconciliation together. If PR1 concludes this grouping under-separates a real risk, it proposes a dedicated administration permission there — this plan does not assume one exists today.
