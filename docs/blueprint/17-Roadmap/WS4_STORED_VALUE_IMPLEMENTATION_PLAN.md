...
| Competitive research | `ADOPT_IMPROVE_REJECT_REGISTER.md` AIR-001 (operation identity/idempotency), AIR-002 (append-oriented consequential facts), AIR-005 (offline bounds/reconciliation); `COMMERCE_WORKFLOW_REFERENCE.md` return/exchange compensation flow. **No stored-value-specific competitive capability matrix rows exist** — recorded as a research gap for the PR1 baseline, not assumed covered |
...
### 3.2 PR1 exit gates — block PR2 from opening; PR1 exists to satisfy these
...
- **G7 — Fixture baseline.** The Demerara Retail Test Group fixture is extended with stored-value programs, instruments, and two-tenant isolation data. PR2 does not assert behavior against it until G7 is recorded.
...
The domain model implements every PDA-DOM-025 (§Core Entities) and PDA-ARC-013 entity; divergence stops for disposition rather than being silently narrowed. (v0.1.0 named only four of PDA-DOM-025's twelve core entities and asserted a transfer/merge deferral that PDA-DOM-025 does not state — its actual §Initial Scope deferrals are coalition programs, cash-equivalent withdrawals, cross-tenant instruments, and regulated wallets, and ADR-0013 explicitly assigns transfer/merge to Commerce. Independent review caught this; the table below is the correction.)
...
| Stored Value Ledger Entry | Implemented — facts Issue, Activate, Load, Reserve, Release, Redeem, Refund, Reverse, Expire, Reinstate, Adjust, Transfer Out, Transfer In |
...
| Manual Adjustment | Implemented — governed by the maker/checker state machine in §7 |
| Transfer or Merge | **Open decision, not deferred by this plan.** PDA-DOM-025 lists it as a core entity and ADR-0013 assigns it to Commerce; this plan does not have authority to narrow that scope on its own (Constitution: no silent contradiction resolution, no scope expansion or reduction for convenience). PR1 records one of: (a) a bounded single-tenant, same-program instrument-to-instrument transfer implementation with its own contract/permission/event, or (b) an explicit deferral proposal routed through PDA-RDM-003 §Change Control (doc + `registry/first-slice.json` together, founder approval since it narrows an ADR-0013-assigned capability). WS4 does not silently ship without one of these two dispositions recorded |
...
| Terms and Disclosure Version | Implemented as a versioned reference surfaced on customer-visible balance/history reads (PDA-DOM-025 §Ledger Model "customer-visible balance, pending value, expiry, and terms") |
...
- **Separation of duties.** Administration, adjustment-request, adjustment-approval, and reconciliation are distinct permissions held by distinct roles in the fixture; tests prove a single role cannot both request and approve the same adjustment, and cannot both adjust and reconcile. (No dedicated stored-value administration permission is currently registered; PR1 either proposes one or records that adjustment/issue/suspend permissions jointly serve that role, rather than asserting a permission that does not exist.)
...
| PR8 | Full evidence matrix run, performance/capacity runs (100k-instrument scale assumption), closeout record, technology-ledger entry, ADR-0013 promotion-case evidence | Scenario 7 end-to-end incl. reversal + reconciliation; every §13 ADR-0013 validation-slice row closed against its bound PR/evidence cell or named as a deferral; DoD (PDA-RDM-007 §6) satisfied per item |
...
| Return-to-original-tender | **Named cross-workstream integration obligation, not WS4 exit evidence.** Interacts with WS3 returns and the WS6 tender seam; WS4 proves only the store-credit destination (below). The original-tender path is recorded as an obligation on the WS3/WS6 seam contracts (§4), not claimed here |
...
| Customer-visible balance/pending-value/expiry/terms | PR7 (operator experience) — **customer-facing** exposure of this data is out of WS4's first-slice scope (no storefront/portal surface exists); PR7 proves the operator-visible equivalent and this gap is named explicitly rather than silently assumed satisfied |
