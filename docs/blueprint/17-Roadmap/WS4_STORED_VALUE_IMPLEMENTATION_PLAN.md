...
PR1 exists to produce G2–G8; they do not uniformly block "PR2" — each names the specific PR it blocks below (G2, G5, G6, G7 block PR2 opening; G4 blocks both PR2 and PR3; G3 and G8 block PR3). PR1's own closeout requires all seven of G2–G8 recorded regardless of which later PR each one blocks.
...
- **Separation of duties.** Adjustment-request (`commerce.stored-value.adjust`), adjustment-approval (`commerce.stored-value.adjustment-approve`, proposed), and reconciliation (`commerce.stored-value.reconcile`) are three distinct, already-nameable permissions held by distinct roles in the fixture; tests prove a single role cannot both request and approve the same adjustment, and cannot both adjust and reconcile. No dedicated stored-value "administration" permission is registered or proposed by this plan — v0.1.0's reference to a distinct administration permission was inaccurate and is removed rather than fixed by inventing one; if a future PR needs one it is proposed there, not assumed here.
...
| Return-to-original-tender | **Not WS4 exit evidence — named unresolved gap, not a claimed obligation on an existing contract.** §4 names no WS3/WS6 tender-seam contract, operation, owner, or closure evidence today — only a generic future composition sentence. WS4 does not manufacture that contract here; it records that return-to-original-tender requires WS3 (returns) and WS6 (tender adapter) to jointly define and own a named seam contract before this validation item can close, and that no such contract currently exists | Blocked on a WS3/WS6-owned contract that does not yet exist; tracked as an open cross-workstream item, not evidenced by WS4 |
...
| Customer-visible balance/pending-value/expiry/terms | PR7 (operator experience) | Operator-visible equivalent only, per `accessibility_and_responsive`/`happy_path` on `commerce.stored-value`. **True customer-facing exposure is out of WS4's first-slice scope** (no storefront/portal surface exists) — this gap is named explicitly, not silently assumed satisfied by the operator-facing evidence |
