...
3. **Application and service names** follow the convention already in the repository: `apps/web`, `apps/native`, `apps/server`, `apps/fumadocs`; deployed services as `meridian-api`, `meridian-worker`, `meridian-postgres`, `meridian-search`, `meridian-observability`.
