...
WS2 PR5 applies this decision explicitly to the `platform.import-export` and `platform.numbering` owners. Their runtime-neutral cores and owner-specific PostgreSQL adapters are authorized only for PDA-RDM-009's controlled prototype and only through the server composition root. This amendment records the previously implicit owner decision; it does not retroactively claim sign-off. Exact-head independent concurrence on the PR5 ownership, transaction, and tenant-isolation evidence is required before PR5 may merge.
...
| Claude Code | WS2 PR5 Import/Export and Numbering owner extension | Pending exact-head re-review | 2026-07-16 | Verify the two explicit owner-specific adapters, server-only composition binding, one-transaction import-reference boundary, separate Catalog/Inventory commands, tenant isolation, migration freshness, and no production-readiness overclaim before PR5 merge. |
