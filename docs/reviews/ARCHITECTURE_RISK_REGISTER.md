...
| RR-004 | Design-token-to-CSS generation pipeline (partially closed; see prose below) | Implementation | Registry-generated CSS for the remaining token groups (`space`, `radius`, `motion`, `screen`, `size`); the non-status semantic aliases are a permanent, recorded exception, not remaining scope |
...
**RR-004 — Partially closed (issue #21).** `scripts/generate_design_tokens_css.py` now generates the `--status-*`, `--border-strong`, and `--border-strong-overlay` custom properties in `packages/ui-web/core/src/styles/globals.css` (one block in `:root`, one in `.dark`, delimited by `/* BEGIN/END generated design tokens */` markers) directly from `registry/design-tokens.json`'s `color.light.*`/`color.dark.*` entries, with `--check` drift detection wired into `bun run gates` and the Documentation Governance workflow (`scripts/test_run_gates.py` verifies the parity). Each generated property's contrast-rationale comment is rendered from that token's `$description` field rather than hand-copied prose, closing TD-004 in full: `registry/design-tokens.json`'s `tokens.color.dark.border-strong` — the one token among the six commented properties that had no `$description` yet — was given one (migrated near-verbatim from the pre-generator CSS comment: "verified at 3.76:1 against the dark card fill... and 4.16:1 against the dark page background"), so every generated property now has a registry-sourced rationale and the generator refuses to run if one goes missing. Verification: a pre/post numeric-token extraction (every hex literal and every `N.NN:1`/`L=`/`~N.NN` contrast figure in `globals.css`) confirmed zero color-value drift and confirmed the eight independently-verified ratios this issue named (border-strong 4.76:1 light / 3.76:1 dark, border-strong-overlay 3.58:1 light / ~4.25:1 dark, status-warning-foreground 7.09:1 light / 10.73:1 dark) survive unchanged; `--check` is idempotent against the committed files (also asserted by `scripts/test_generate_design_tokens_css.py`'s `RepositoryStateTests`). Not closed in full: the non-status semantic aliases (`--action-primary`, `--surface-*`, `--text-*`, `--focus-ring`) remain deliberately hand-maintained per the fifth-audit F-H-003 exception (a permanent, reviewed divergence, not remaining generator scope), and the registry's `space`, `radius`, `motion`, `screen`, and `size` groups have no CSS custom-property generation at all yet — so the registry is not yet "the single styling source in code" end to end, and RR-004 stays in the Currently Open Risks list with its narrowed remaining scope recorded above. Reopen at full scope only if a future change needs registry-driven CSS for those remaining groups; reopen this specific slice only if new evidence shows a generated value or comment is wrong.
...
| TD-004 | **Closed 2026-07-24 (issue #21).** `scripts/generate_design_tokens_css.py` generates the `status-*`/`border-strong`/`border-strong-overlay` CSS custom properties from `registry/design-tokens.json`, with `--check` drift detection in `bun run gates` and Documentation Governance CI. Evidence: `scripts/test_generate_design_tokens_css.py` (14/14); `python scripts/generate_design_tokens_css.py --check` and `python scripts/validate_docs.py`/`validate_ui_governance.py` all green; pre/post numeric-token diff confirmed zero hex/contrast-ratio drift. See RR-004. | Pipeline (PDA-UX-023) not yet built; literals unblock UI work | Token-generation pipeline lands (PDA-UX-023) | Implementation |
...
- **0.14.0 (2026-07-24):** Closed TD-004 and partially closed RR-004 (issue #21) — `scripts/generate_design_tokens_css.py` now generates the `status-*`/`border-strong`/`border-strong-overlay` custom properties in `globals.css` from `registry/design-tokens.json`, with `--check` drift detection in `bun run gates` and Documentation Governance CI, and contrast-rationale comments sourced from each token's `$description`. RR-004's remaining scope (non-generated `space`/`radius`/`motion`/`screen`/`size` groups; the F-H-003 non-status alias exception stays permanent by design) is recorded and RR-004 stays in the Currently Open Risks list with narrowed blocking text.
