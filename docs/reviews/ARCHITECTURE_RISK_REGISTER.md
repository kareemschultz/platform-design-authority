...
**RR-005 — Open; catalog inventory advanced, PDA-UX-028 gate not met.** Issue #22's original "8 primitives" baseline was already stale by a 2026-07-19 audit comment recording 14 at WS3 head `0b84b407`; `packages/ui-web/core/src/components/` now holds 17 (`alert`, `badge`, `button`, `card`, `checkbox`, `dialog`, `dropdown-menu`, `input`, `label`, `metric-card`, `metric-empty-state`, `select`, `separator`, `sheet`, `skeleton`, `sonner`, `table`). This pass (2026-07-24) found `Dialog`, `DropdownMenu`, `Sheet`, and `Toaster (Sonner)` already built with live call sites (7, 2, 1, and 1 respectively, plus the globally-mounted toaster) but not yet catalogued in `PREFERRED_COMPONENT_CATALOG.md` — a documentation gap, now closed by adding their Prototype Approved entries in this same change, version 0.9.0. `Select` and `Table` were already catalogued by prior PRs (#231/#233). Against PDA-UX-028's (`SHADCN_CONFIGURATION_DECISION_MATRIX.md`) Prototype Gates list — Dialog, Menu, Popover, Select, Combobox, Field, Tabs, Toast, Drawer, data grid, complex multi-select, chart, and POS controls — Popover, Combobox, Field, Tabs, complex multi-select, chart, and POS controls remain genuinely unbuilt; repository search confirmed none has a forcing call site in `apps/web` today, so building any of them now would be speculative under this catalog's own no-build-ahead-of-need precedent (the existing `Select` entry's rejection of Combobox for the same reason). Recorded as `Needed` in the catalog's new "Needed" section rather than closed or built. RR-005 stays **Open**: PDA-UX-028 itself remains Draft regardless of component count, since its Prototype Gates also require accessibility, zoom, RTL, theming, Storybook, Playwright, and real-device evidence that this pass did not attempt. Platform-wide sequencing continues under issue #110, per the existing issue #22 comment designating RR-005 as the primitive/catalog risk record and #110 as its Phase 1 parent. Reopen-irrelevant since still Open; re-scope only if a real call site newly forces one of the `Needed` families, or when #110 sequences the next batch.
