...
|---|---|---|---|
| GAP-001 Missing declared directories | Accepted | Closed for structure; content remains planned | Planned indexes now exist for all declared sections; `docs/blueprint/17-Roadmap/BLUEPRINT_AND_DELIVERY_ROADMAP.md` and `docs/blueprint/17-Roadmap/RATIFICATION_WAVES.md` define delivery and review sequencing. |
| GAP-002 Missing extensible metadata and custom fields | Accepted | Architectural blocker closed; prototype validation open | `docs/blueprint/01-Platform/EXTENSIBLE_METADATA_AND_CUSTOM_FIELDS.md` and ADR-0008 define the kernel primitive and hybrid storage strategy. |
| GAP-003 Unresolved Party model | Accepted | Architectural blocker closed; schema validation open | `docs/blueprint/01-Platform/PARTY_AND_RELATIONSHIP_MODEL.md`, ADR-0007, capability map, and dependency matrix establish canonical Party ownership. |
...
| GAP-005 Missing AI engine owner | Accepted | Closed at engine-boundary level; full AI book open | `docs/blueprint/03-Business-Engines/AI_ORCHESTRATION_ENGINE.md`, engines overview, capability map, and dependency matrix register the owner. |
...
| AMB-004 Loyalty has no owner | Accepted | Closed through ADR-0009 and `docs/blueprint/03-Business-Engines/LOYALTY_ENGINE.md`. |
| AMB-005 Module definition drift | Accepted | Open for next Foundation cleanup. |
| AMB-006 Better Auth evidence quality | Accepted | Closed through `docs/blueprint/19-Appendices/BETTER_AUTH_VERIFICATION-2026-07-10.md`; re-verification remains required before contracting or implementation. |

## Missing Capabilities and Services

| Area | Disposition | Status and Evidence |
|---|---|---|
| Sequence and numbering service | Accepted | Closed architecturally: `docs/blueprint/01-Platform/SEQUENCE_AND_NUMBERING_SERVICE.md`. |
| Webhook management | Accepted | Closed architecturally: `docs/blueprint/07-Developer-Platform/WEBHOOKS_AND_EVENT_DELIVERY.md`. |
| Import, export, and migration platform | Accepted | Closed architecturally: `docs/blueprint/01-Platform/IMPORT_EXPORT_AND_DATA_MIGRATION.md`. |
| Rate limiting and quotas | Accepted | Closed architecturally: `docs/blueprint/01-Platform/RATE_LIMITS_QUOTAS_AND_ABUSE_CONTROLS.md`. |
| Privacy rights and retention workflows | Accepted | Closed architecturally: `docs/blueprint/11-Security/PRIVACY_RIGHTS_AND_RETENTION.md`. |
| Collaboration primitives | Accepted | Closed architecturally: `docs/blueprint/01-Platform/COLLABORATION_PRIMITIVES.md`. |
| Tenant-facing recurring commerce | Accepted | Closed at domain-outline level: `docs/blueprint/04-Business-Domains/RECURRING_COMMERCE_AND_MEMBERSHIPS.md`. |
| Fiscalization | Accepted | Closed at engine-boundary level: ADR-0010 and `docs/blueprint/03-Business-Engines/FISCALIZATION_AND_STATUTORY_REPORTING_ENGINE.md`. Jurisdiction specifications remain open. |
| Storefront scope | Accepted | Closed through ADR-0012 and `docs/blueprint/04-Business-Domains/STOREFRONT_AND_DIGITAL_COMMERCE.md`. |
| Anomaly and fraud engine | Accepted for registration, deferred for implementation | Ownership closed through `docs/blueprint/11-Security/RISK_FRAUD_AND_ANOMALY.md`; advanced models remain deferred. |
| Semantic search ownership | Accepted | Closed at architecture-outline level through `docs/blueprint/10-Data/SEARCH_RELEVANCE_AND_SEMANTIC_RETRIEVAL.md`. |
| Billing-provider ADR | Accepted | Closed as a strategy decision through ADR-0011 and `docs/blueprint/13-Commercial/BILLING_PROVIDER_AND_REGIONAL_PAYMENT_RAILS.md`; final provider selection remains deferred until the operating legal entity is confirmed. |

## Commercial and Regulatory Verification

Dated primary-source evidence is recorded in:

- `docs/blueprint/19-Appendices/BETTER_AUTH_VERIFICATION-2026-07-10.md`
- `docs/blueprint/19-Appendices/REGIONAL_PAYMENTS_PRIVACY_AND_FISCALIZATION_VERIFICATION-2026-07-10.md`
...
The governing plan is now `docs/blueprint/17-Roadmap/RATIFICATION_WAVES.md`:
