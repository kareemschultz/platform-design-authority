...
import { DECIMAL_QUANTITY_PATTERN, POSITIVE_DECIMAL_QUANTITY_PATTERN } from ".";
...
describe("Inventory quantity contract", () => {
	...
	test("requires positive directional transfer quantities", () => {
		...
		expect(POSITIVE_DECIMAL_QUANTITY_PATTERN.test("0.000000")).toBe(false);
		expect(POSITIVE_DECIMAL_QUANTITY_PATTERN.test("-1")).toBe(false);
	});
});
